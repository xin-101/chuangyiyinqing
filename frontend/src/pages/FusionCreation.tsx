  ...
  return (
    <div className="max-w-5xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        ...
        <p className="text-slate-400 text-sm mt-1">图文匹配 · 视频脚本 · 全案生成</p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          <div className="glass-card p-5 space-y-4">
            {tab === 'match' && (
              <>
                <p className="text-xs text-slate-400">粘贴文章内容，AI 自动生成配套插图</p>
                <textarea
                  value={matchText}
                  onChange={(e) => setMatchText(e.target.value)}
                  placeholder="粘贴文章或文案内容..."
                  className="w-full bg-slate-100 border border-slate-200 rounded-lg p-3 text-sm text-slate-700 resize-none h-40 focus:outline-none focus:border-sky-500/50 placeholder:text-slate-300"
                />
              </>
            )}
            {tab === 'video' && (
              <>
                <p className="text-xs text-slate-400">输入主题，一键生成完整视频制作方案</p>
                <input
                  value={videoTopic}
            ...
            {tab === 'pipeline' && (
              <>
                <p className="text-xs text-slate-400">输入一个灵感，同时生成文案+配图+视频脚本</p>
                <textarea
                  value={pipelineIdea}
        ...
        >
          {result ? (
            ...
            <div className="glass-card p-10 text-center h-full flex flex-col items-center justify-center">
              ...
              <p className="text-slate-300 text-sm">选择左侧模式，输入内容后开始创作</p>
            </div>
          )}
        </motion.div>
      </div>
